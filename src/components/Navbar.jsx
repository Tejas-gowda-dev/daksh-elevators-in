    ...
    <header
      ...
      <nav className="container  max-w-[88rem] flex items-center justify-between h-20">
        ...
        <Link href="/" className="flex items-center gap-1 group">
          <img src={Logo} alt="Passenger Elevator by Daksh Elevators" className="h-20 w-auto" />
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-0">
          {NAV_LINKS.map((link) => (
            ...
            >
              <Link
                href={link.href}
                className={`flex items-center gap-1 px-3 py-2 text-[13px] font-semibold tracking-wide transition-colors duration-300 ${location === link.href
                    ? "text-[#0077CC]"
                ...
                <div key={link.href} className="border-b border-gray-100 last:border-b-0">
                  ...
                  {link.children ? (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-4 py-3 text-base font-semibold rounded-lg transition-colors ${location === link.href
                          ? "text-[#0077CC]"
                          : "text-gray-700 hover:text-[#0077CC]"
                        }`}
                    >
                      ...
                      <ChevronDown
                        className={`w-5 h-5 transition-transform duration-300 ${openDropdown === link.href ? "rotate-180" : ""
                          }`}
                      />
                    </button>
                  ) : (
                    <Link
                      href={link.href}
                      className={`block px-4 py-3 text-base font-semibold rounded-lg transition-colors ${location === link.href
                          ? "text-[#0077CC]"
                          : "text-gray-700 hover:text-[#0077CC]"
                        }`}
                    >
